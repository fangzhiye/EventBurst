...
def match_frames(left_frame,right_frame,frame_id,T=0.01):#前一帧为left_fram,后一帧为right_fram
    #T为相似度的阈值
    ...
    for i in range(len(row_ind)):
        ...
        if(old_key not in ret.keys()):#新事件的开始
            ret[new_key] = []
            ret[new_key].append(comm_left)
            ret[new_key].append(comm_right)
        else:#匹配到旧事件
            ret[old_key].append(comm_right)
            ret[new_key] = ret.pop(old_key)
#%%
num_frames = len(frames)
#key为"frame_id + endcomm_id":[{},{},{},"每个元素为事件"]
for i in range(num_frames-1):
    match_frames(frames[i],frames[i+1],i+10)
